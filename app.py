'''
Эта функция разбивает входную строку на слова и сопоставляет их с определенными значениями
'''
def lex(s):
    a = s.split()
...
'''
 Эта функция преобразует список пар  в итоговый список,
 который можно использовать для арифметических операций.
'''
def syntacsis(a):
    r = []
    numbers = False
...
'''
Это главная функция, которая принимает входную строку,
анализирует её и возвращает результат арифметического выражения
'''
def calc(s):
    a = lex(s)
    if 'Unknown' in a:
...
s = 'пять плюс два умножить на три минус один'
s = 'скобка открывается пять плюс два скобка закрывается умножить на три минус один'
print(calc(s))
